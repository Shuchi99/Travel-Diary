...
const ImageSelector = ({image, setImage, handleDeleteImg}) => {
  ...
  return (
    <div>
      ...
      {!image ? <button className='w-full h-[220px] flex flex-col items-center justify-center gap-4 bg-slate-50 rounded border border-slate-200/50 cursor-pointer'
      onClick={() => onChooseFile()}>
        <div className='w-14 h-14 flex items-center justify-center bg-cyan-50 rounded-full border border-cyan-100'>
            <FaRegFileImage className='text-xl text-cyan-500'></FaRegFileImage>
        </div>
